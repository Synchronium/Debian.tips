...
- **`autoremove`** takes away packages *you never asked for*: the dependencies something else
  dragged in and no longer needs. It removes different packages, rather than removing them more
  thoroughly.
...
The `*` means "and its configuration", and that mark is the only thing separating the two plans.
It is also easy to skim past on a confirmation prompt you are about to answer `y` to, which is
why it is worth getting into the habit of using `--dry-run` first.

`nano` ships `/etc/nanorc`, and removing the editor does not touch it:
...
`remove` is for a package you might reinstall, where the settings you spent an afternoon on are
worth keeping. `purge` is for one you are finished with, including one you are about to reinstall
*because its configuration is broken*, where leaving the old file in place means reinstalling
changes nothing.

## rc: removed but not purged
...
`cowsay-off` depends on it, so apt marked it as automatic, and `autoremove` later reads that
mark to decide whether or not the package stays.
...
`autoremove` proposing a long list of kernels, a desktop environment, or half of X nearly always
means a metapackage that was holding those together got removed, so everything under it is now
technically unreferenced. The fix for this is a single command.
...
package for its own sake, and `autoremove` stops proposing it, permanently.
...
"Uninstall properly" usually means that last one. Use it for software you installed to try out
and did not keep.
...
- **Your home directory.** `~/.config`, `~/.bashrc`, a dotfile the program wrote on first run:
  none of it belongs to the package, so none of it is removed.
...
- **Anything you edited into place by hand.** dpkg never registered it, so dpkg will not remove
  it.
...
"I want my settings back later", and if that is true you know it at the time. The risk of
keeping them only shows up much later, when a stale config file outlives your memory of writing
it and breaks a reinstall two years on.

**Run `autoremove` when apt suggests it**, not on a schedule and never with `-y` on a machine
you care about. Read its output in full, because the answer depends on marks you may have set
years ago.

**Use `--dry-run` first** on all three. Every example above is one, and none of them cost
anything.

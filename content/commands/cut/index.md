...
`cut` has two sharp edges worth knowing up front. First, the delimiter is exactly **one
character** — `cut -d: -f2` on `a::b` sees two single-colon splits, not one double-colon split,

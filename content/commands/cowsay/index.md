...
it. That is the whole program. It matters here for the same reason `wc` does: it is a filter,
it reads standard input, and it composes with everything else on this site.
...
**The one behaviour that surprises everyone is that `cowsay` reflows.** It does not print your
text, it re-wraps it: line breaks in the input are discarded, the whole thing is treated as one

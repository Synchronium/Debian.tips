...
`dpkg` is the program that actually installs Debian packages. It unpacks a `.deb`, runs its
...
The difference between them is worth stating precisely, because it explains most of what is
frustrating about `dpkg` used alone: **`dpkg` knows about packages, and `apt` knows about
repositories.** `dpkg` works with one file you already have. It cannot download anything, does
not know what a repository is, and — the important one — **will not resolve dependencies**. It
...
## What dpkg is genuinely better at
...
The first letter is what you *wanted*, the second is what is actually true. When they disagree,
...
install either completes or does not start. Reach for `dpkg -i` when apt is not available or not
working — which is a real situation, and the reason to know it exists.

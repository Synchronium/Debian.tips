...
prompt and nothing kept, which is why `-i`, `-n` and `--backup` exist. The exception is a
destination that is currently being executed, where the write is refused with

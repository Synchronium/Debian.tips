...
Both mean the same thing. Debian 13 prints the second, and almost every answer you will find
online was written for the first.

## Either you have no key, or the wrong one
...
The network is fine and so is the repository. apt fetched the index perfectly well, then
refused to trust it.
...
If that fingerprint is not the one in the error, you have the wrong key. That happens more often
than having no key at all, whenever a vendor rotates its signing key or when instructions were
copied from an older version of a vendor's documentation.
...
## Install the repository's current key

Fetch it and put it where the source expects it. The vendor publishes the key; use their URL
rather than a keyserver:
...
that is all it takes, and the packages it offers become available:
...
[Adding a third-party repository safely](/debian/third-party-repositories/) covers it in full. If you are
following instructions that use `apt-key`, they predate this change, and the rest of their
advice may be equally old.

## Keeping repositories verifiable

...
description: Review and merge Dependabot pull requests on debian.tips — grouped minor/patch bumps, individual majors, GitHub Actions and devcontainer updates. Use when asked to deal with, review, triage or merge dependency PRs or Dependabot PRs. Explains what each ecosystem can actually break here.
---
...
| **typescript** | It is the only static check in the repo — there is no linter. A major can surface real errors that were always there, which is worth fixing rather than suppressing. |
...
| **remark/rehype/unified** | Markdown rendering. Callouts, heading slugs and autolinks are custom plugins — `npx vitest run test/markdown.test.ts` covers them, and `npm run check`'s linkcheck catches broken `#fragment` targets if slugging changed. |
...
If a bump has to be held back, say why in the PR rather than closing it silently — Dependabot

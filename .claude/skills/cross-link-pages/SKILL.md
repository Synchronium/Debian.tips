...
What the audit deliberately does not report is one-way `related:` — A listing B while B does not
...
4. **`related:`-only is acceptable** when the connection is real but no existing sentence sets it
   up — `find` → the bulk-rename recipe. It is the weaker fix; don't reach for it first.
...
needs both — it is entirely possible to fix an orphan with a link that 404s, and the audit alone

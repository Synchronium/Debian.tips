...
So the order is a parameter. `alpha` stays the default and the reference — when a shuffled run
fails and the default passes, the difference is the ordering rather than a regression. CI runs the
...
and the pages it reorders are the ones the author just changed — and a push to `main` always
replays everything, so nothing is deployed on the strength of a partial run.
...
The measurements above argue against reaching for it early. The ceiling is roughly 3.5x and is set
...
them hides it. Splitting the work is not a cheaper version of the contamination test — it is less
of one.

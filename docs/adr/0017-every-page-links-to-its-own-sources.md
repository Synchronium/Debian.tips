...
empty-handed from the one block whose job is to make the claim checkable. The two sentences are
chosen on `PageSources.hasSkipFile`, and `test/sourceLinks.test.ts` asserts that the file each
sentence names is one the page lists.

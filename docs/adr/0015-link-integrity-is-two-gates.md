...
question the new page answers, rather than another `related:` entry. That editorial half is
...
the paths against the working tree rather than fetching anything. Use the same pattern if another
class of external link turns up, and validate what generates the URL rather than the URL. Links

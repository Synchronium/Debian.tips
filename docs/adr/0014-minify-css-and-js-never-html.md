...
The CSS source map names its source `site.css` rather than the hashed output name — devtools would
otherwise label the original `site.<hash>.css`, which means "the minified file" everywhere else on
the site.
